...
namespace Funkin.NET.Content.osu.Graphics.Containers
{
    ...
    public class ParallaxContainer : Container, IRequireHighFrequencyMousePosition
    {
        ...
        {
            parallaxEnabled = config.GetBindable<bool>(FunkinSetting.MenuParallax);
        ...
        protected override void Update()
        {
            ...
            if (parallaxEnabled.Value)
            {
                ...
                content.Position = Interpolation.ValueAt(elapsed, content.Position, offset, 0, parallax_duration,
                    Easing.OutQuint);
                content.Scale = Interpolation.ValueAt(elapsed, content.Scale, new Vector2(1 + Math.Abs(ParallaxAmount)),
                    0, 1000, Easing.OutQuint);
            }
